...
//    var systemName: CategoriesEnum
    var systemName: ProductCategory
    
    var body: some View {
        Image(systemName: systemName.imageName)
            .foregroundStyle(.fillsSecondary)
    }
}

#Preview {
    let category = ProductCategory(rawValue: "Sunglasses")
    
    CategoriesImage(systemName: category ?? .beauty)
}
